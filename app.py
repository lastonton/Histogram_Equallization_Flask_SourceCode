...
def visualize_histogram(histogram, output='static\\histogram\\histogram.png'):
    hist_data = pd.DataFrame(
        {'intensity': list(range(256)), 'frequency': histogram})
    sns_hist = sns.barplot(x='intensity', y='frequency',
                           data=hist_data, color='blue')
...
def histogram():
    ...
    if request.method == 'POST':
        ...
        grayscale_img.save(os.path.join(
            app.config['SAVE'], 'grayscale_'+filename))
        new_img_2 = cv2.equalizeHist(img_gray)

        histogram = create_histogram(img_gray)
        hist_img_path = visualize_histogram(histogram, output=os.path.join(
            app.config['HISTOGRAM'], 'histogram_' + filename))

        histogram_after_process = create_histogram(new_img_2)
        hist_img_path = visualize_histogram(histogram_after_process, output=os.path.join(
            app.config['HISTOGRAM'], 'histogram_after_' + filename))
        ...
        histogram_before_show = os.path.join(
            app.config['HISTOGRAM'], 'histogram_' + filename)
        print(histogram_before_show)
        histogram_after_show = os.path.join(
            app.config['HISTOGRAM'], 'histogram_after_' + filename)
